-- =====================================================
-- SCRIPT DE MIGRAÇÃO PARA ESTRUTURA EXISTENTE
-- =====================================================
...
BEGIN
    ...
    -- Migrar status para novos valores
    IF column_exists('consultations', 'status') THEN
        ...
        SET status = CASE 
            ...
            ELSE status
        END;
        RAISE NOTICE 'Status das consultas migrados para novos valores';
    END IF;
...
INSERT INTO users (id, email, name, phone, is_doctor, specialty, subscription_type) 
SELECT 
...
INSERT INTO patients (id, doctor_id, name, email, phone, city, status) 
SELECT 
...
-- Remover colunas antigas que foram migradas (opcional - comentado por segurança)
/*
-- Descomente estas linhas APÓS confirmar que a migração funcionou
ALTER TABLE consultations DROP COLUMN IF EXISTS modality;
ALTER TABLE consultations DROP COLUMN IF EXISTS scheduled_date;
ALTER TABLE consultations DROP COLUMN IF EXISTS scheduled_time;
ALTER TABLE consultations DROP COLUMN IF EXISTS audio_url;
ALTER TABLE consultations DROP COLUMN IF EXISTS transcription;

ALTER TABLE transcriptions DROP COLUMN IF EXISTS content;
ALTER TABLE transcriptions DROP COLUMN IF EXISTS confidence_score;

ALTER TABLE audio_files DROP COLUMN IF EXISTS file_name;
ALTER TABLE audio_files DROP COLUMN IF EXISTS file_size;
ALTER TABLE audio_files DROP COLUMN IF EXISTS file_url;
...
IMPORTANTE: Se tudo estiver funcionando, você pode descomentar as linhas de limpeza
para remover as colunas antigas que não são mais necessárias.
*/
