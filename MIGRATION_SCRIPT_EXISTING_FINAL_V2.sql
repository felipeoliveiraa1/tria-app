-- =====================================================
-- SCRIPT DE MIGRAÇÃO PARA ESTRUTURA EXISTENTE - VERSÃO FINAL V2
-- =====================================================
...
-- NOTA: Não podemos inserir com ID específico devido à foreign key para auth.users
-- Vamos apenas atualizar usuários existentes ou criar sem ID específico
DO $$
BEGIN
    -- Verificar se já existe um usuário médico
    IF NOT EXISTS (SELECT 1 FROM users WHERE email = 'medico@tria.com') THEN
        -- Se não existir, vamos criar um usuário de exemplo (sem ID específico)
        -- O ID será gerado automaticamente pelo Supabase
        INSERT INTO users (email, name, phone, is_doctor, specialty, subscription_type) 
        VALUES (
            'medico@tria.com',
            'Dr. Felipe Porto',
            '(11) 99999-9999',
        ...
        RAISE NOTICE 'Usuário médico de exemplo criado';
    ELSE
        -- Se já existir, atualizar os campos
        UPDATE users 
        SET 
...
-- NOTA: Usar o ID real do usuário médico existente
DO $$
DECLARE
    doctor_id uuid;
BEGIN
    ...
    IF doctor_id IS NOT NULL THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM patients WHERE email = 'marcos.paulo@triacompaby.com.br') THEN
            ...
            RAISE NOTICE 'Paciente Marcos Paulo criado';
        ELSE
        ...
        IF NOT EXISTS (SELECT 1 FROM patients WHERE email = 'jeferson@triacompany.com.br') THEN
            ...
            RAISE NOTICE 'Paciente Jeferson criado';
        ELSE
            RAISE NOTICE 'Paciente Jeferson já existe';
        END IF;
    ELSE
        RAISE NOTICE 'Usuário médico não encontrado, pacientes não serão criados';
    END IF;
END $$;
