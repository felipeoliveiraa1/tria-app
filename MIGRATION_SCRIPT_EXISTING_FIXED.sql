-- =====================================================
-- SCRIPT DE MIGRAÇÃO PARA ESTRUTURA EXISTENTE - VERSÃO CORRIGIDA
-- =====================================================
...
BEGIN
    ...
    IF column_exists('consultations', 'status') THEN
        -- Primeiro, alterar a constraint para aceitar os novos valores
        BEGIN
            ...
            ALTER TABLE consultations ADD CONSTRAINT consultations_status_check 
                CHECK (status = ANY (ARRAY['agendada'::text, 'em_andamento'::text, 'concluida'::text, 'cancelada'::text, 'CREATED'::text, 'RECORDING'::text, 'PROCESSING'::text, 'COMPLETED'::text, 'ERROR'::text, 'CANCELLED'::text));
            
            RAISE NOTICE 'Constraint de status atualizada para aceitar novos valores';
